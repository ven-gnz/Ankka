...
glm::vec3 GltfAnimationChannel::getTranslation(float time)
{
	...
	if (prevTimeIndex == nextTimeIndex) return mTranslations.at(prevTimeIndex);
...
{
	// sanity check
	if (mScaling.size() == 0) return glm::vec3(1.0f);

	if (time < mTimings.at(0)) return mScaling.at(0);
	if (time > mTimings.at(mTimings.size() - 1)) return mScaling.at(mScaling.size() - 1);
	...
	if (prevTimeIndex == nextTimeIndex) return mScaling.at(prevTimeIndex);

	glm::vec3 finalScale = glm::vec3(1.0f);

	switch (mInterType)
	{
	case EInterpolationType::STEP: finalScale = mScaling.at(prevTimeIndex); break;
	case EInterpolationType::LINEAR: {
...
glm::quat GltfAnimationChannel::getRotation(float time)
	...
	if (mRotations.size() == 0) return glm::identity<glm::quat>();
	if (time < mTimings.at(0)) return mRotations.at(0);
	if (time > mTimings.at(mTimings.size() - 1)) return mRotations.at(mTimings.size() - 1);
	...
	if (prevTimeIndex == nextTimeIndex) return mRotations.at(prevTimeIndex);

	glm::quat finalRotate = glm::quat(1.0f, 0.0f, 0.0f, 0.0f);

	switch (mInterType)
	{
	case EInterpolationType::STEP: { finalRotate = mRotations.at(prevTimeIndex); break; }
	case EInterpolationType::LINEAR: {
		float interpolatedTime = (time - mTimings.at(prevTimeIndex)) / (mTimings.at(nextTimeIndex) - mTimings.at(prevTimeIndex));
